...
#         card = f"datacard_hmmg_runII_comb_{mass}.root"
#         card2root = card.replace(".txt", ".root")
#         ext = "_" + card2root.replace("datacard_", "").replace(".root", "")
#         task.append(f"combine {card2root} -M AsymptoticLimits -n {ext} -m 125.38 --cminDefaultMinimizerStrategy 0 &> ../logger/AsymptoticLimits{ext}_unblind.txt")
#     submit(task)  
...
with cd("./cards"):
    mass_interp = np.linspace(120, 130, 11, endpoint=True).astype(int)
    for mass in  mass_interp:
        execute(f"combine -M GenerateOnly datacard_hmmg_runII_comb_{mass}.txt -t -1 --saveToys --expectSignal=1 --expectSignalMass={mass} -m {mass}")
...
#         task.append(f"combine datacard_hllg_runII_comb_{mass}.root -M Significance -n _comb_{mass}_expSignal1 -m {mass} -t -1 --toysFile higgsCombineTest.GenerateOnly.mH{mass}.123456.root &> ../logger/Significance_comb_{mass}_expSignal1.txt")
#         task.append(f"combine datacard_hllg_runII_comb_{mass}.root -M Significance -n _comb_{mass}_expectSignalMass125 -m {mass} -t -1 --toysFile higgsCombineTest.GenerateOnly.mH125.123456.root &> ../logger/Significance_comb_{mass}.txt")
