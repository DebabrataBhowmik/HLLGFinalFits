...
// bool BLIND = true;
// string norm_range = ""; // "full" or "left,right"
...
// void runFit(RooRealVar* mass, RooAbsPdf* pdf, RooDataSet* data, double* NLL, int* stat_t, int MaxTries){
//     mass->setRange("full", mllg_low, mllg_high);
//     mass->setRange("left", mllg_low, blind_low);
//     mass->setRange("right", blind_high, mllg_high);
...
//             *data, Range(norm_range.c_str()),
//             Save(1), Minimizer("Minuit", "minimize"),
//             SumW2Error(true)
//         );
...
//         *data, Range(norm_range.c_str()),
//         Save(1), Strategy(1),
//         Minimizer("Minuit","minimize"), SumW2Error(true), PrintLevel(-1)
//     );
...
//                 *binnedtoy, Range(norm_range.c_str()),
//                 Save(1), Strategy(1),
//                 Minimizer("Minuit", "minimize"), PrintLevel(-1)
//             );
//             //,Optimize(0));
...
//                 *binnedtoy, Range(norm_range.c_str()),
//                 Save(1), Strategy(1), SumW2Error(true), //FIXME
//                 Minimizer("Minuit", "minimize"), PrintLevel(-1)
//             );
...
//             pdf->fitTo(
//                 *binnedtoy, Range(norm_range.c_str()),
//                 Minimizer("Minuit", "minimize"),
//                 PrintLevel(-1)
//             );

//             RooPlot* plot_t = mass->frame();
//             binnedtoy->plotOn(plot_t);
//             pdf->plotOn(plot_t); //,NormRange("fitdata_1,fitdata_2"));
...
//     mass->setRange("right", blind_high, mllg_high);
//     // if (BLIND) {
//     //     data->plotOn(plot, Range("left"),  NormRange("left,right"));
//     //     data->plotOn(plot, Range("right"), NormRange("left,right"));
//     //     data->plotOn(plot, Binning(mllg_high - mllg_low), Invisible());
//     // }
//     // else data->plotOn(plot, Range("full"),  NormRange("full"));
//     data->plotOn(plot);

//     TCanvas* canv = new TCanvas("canv", "", 700, 600);
//     pdf->plotOn(plot); //,NormRange("fitdata_1,fitdata_2"));
//     bool paramoncanv = false;
//     if (paramoncanv)
//         pdf->paramOn(plot, Layout(0.17, 0.93, 0.89), Format("NEA", AutoPrecision(1)));
//     if (BLIND)
...
// int getBestFitFunction(RooRealVar* mass, RooMultiPdf* bkg, RooDataSet* data, RooCategory* cat, bool silent = false){
...
// 		int fitStatus = 1;
// 		runFit(mass, bkg->getCurrentPdf(), data, &minNll, &fitStatus, /*max iterations*/5);
...
//     mass->setRange("right", blind_high, mllg_high);
//     if (BLIND){
//         data->plotOn(plot, Range("left"),  NormRange("left,right"), XErrorSize(0.0001));
//         data->plotOn(plot, Range("right"), NormRange("left,right"), XErrorSize(0.0001));
//         // data->plotOn(plot, Binning(mllg_high - mllg_low), Invisible());
//     }
//     else
//         data->plotOn(plot, Range("full"),  NormRange("full"), XErrorSize(0.0001));
//     // data->plotOn(plot);
...
//         pdfs->getCurrentPdf()->fitTo(*data, Minimizer("Minuit", "minimize"));
...
//     string fmultipdf;
//     string multipdf_dir = (BLIND) ? "multipdf" : "multipdf_unblind";
//     if (singleCategory > -1){
//         fmultipdf = Form("./%s/CMS_HLLG_multipdf_%s_%s.root", multipdf_dir.c_str(), ext.c_str(), HLLGCats[startingCategory].c_str());
//         cout << Form("[INFO] Save multiPdf model: %s", fmultipdf.c_str()) << endl;
//     }
//     else{
//         fmultipdf = Form("./%s/CMS_HLLG_multipdf_%s.root", multipdf_dir.c_str(), ext.c_str());
//         cout << Form("[INFO] Save multiPdf model: %s", fmultipdf.c_str()) << endl;
//     }
...
//     mass->setRange("left", mllg_low, blind_low);
//     mass->setRange("right", blind_high, mllg_high);
//     norm_range = (BLIND) ? "left,right" : "full";
...
//         // RooDataSet* dataBlind = dataFull; 
//         RooDataSet* dataBlind; //! modified by CH -> gives strange fitting
//         if (BLIND)
//             dataBlind = (RooDataSet*) dataFull->reduce(CutRange(norm_range.c_str()));
        
//         // binned dataset
//         RooDataSet* data;
//         string thisdataBinned_name = Form("roohist_data_mass_%s", catname.c_str());
//         RooDataHist thisdataBinned(thisdataBinned_name.c_str(), "data", *mass, (BLIND) ? *dataBlind : *dataFull);
...
//                     runFit(mass, bkgPdf, data, &thisNll, &fitStatus, /*max iterations*/5);
//                     if (fitStatus != 0)
...
//                     //RooFitResult *fitRes;
//                     int fitStatus = 0;
//                     runFit(mass, bkgPdf, data, &thisNll, &fitStatus, /*max iterations*/5);//bkgPdf->fitTo(*data,Save(true),Minimizer("Minuit","minimize"));
...
//         int bestFitPdfIndex = getBestFitFunction(mass, pdf, data, &catIndex, !verbose);
...
//         // mass->setRange("signalReg", blind_low, blind_high);
//         // RooAbsPdf* bestPdf = (RooAbsPdf*) storedPdfs.at(bestFitPdfIndex);
//         // RooAbsReal* integral_full = bestPdf->createIntegral(*mass, *mass, "full");
//         // RooAbsReal* integral_blind1 = bestPdf->createIntegral(*mass, *mass, "unblindReg_1");
//         // RooAbsReal* integral_blind2 = bestPdf->createIntegral(*mass, *mass, "unblindReg_2");
//         // RooAbsReal* integral_sig = bestPdf->createIntegral(*mass, *mass, "signalReg");
//         // RooDataSet* sideband = (RooDataSet*) data->reduce(Form("(CMS_higgs_mass > %d && CMS_higgs_mass < %d) || (CMS_higgs_mass > %d && CMS_higgs_mass < %d)", mllg_low, blind_low, blind_high, mllg_high));
//         // float nbkg_blind = sideband->sumEntries() * (integral_full->getVal()/(integral_full->getVal() - integral_sig->getVal()));
//         // cout << "integral: " << nbkg_blind << endl;
//         // cout << "yields: " << data->sumEntries() << endl;

//         mass->setBins(nBinsForMass);
//         RooDataHist dataBinned(Form("roohist_data_mass_%s", catname.c_str()), "data", *mass, (BLIND) ? *dataBlind: *dataFull);
...
//         outputws->import((BLIND) ? *dataBlind: *dataFull, Rename(Form("data_mass_%s", catname.c_str())));
//         plot_best(
